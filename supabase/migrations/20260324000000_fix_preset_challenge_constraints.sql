/*
  # Fix Preset Challenge Constraints

  ## Changes
  1. Expand duration_days check to allow 21 and 30 day challenges
  2. Add 'no_spend' to task_type allowed values
  3. Re-run preset challenge creation now that constraints allow it
*/

-- Fix duration_days constraint (was only 7 or 14)
ALTER TABLE challenges DROP CONSTRAINT IF EXISTS challenges_duration_days_check;
ALTER TABLE challenges ADD CONSTRAINT challenges_duration_days_check
  CHECK (duration_days IN (7, 14, 21, 30));

-- Fix task_type constraint (was missing 'no_spend')
ALTER TABLE tasks DROP CONSTRAINT IF EXISTS tasks_task_type_check;
ALTER TABLE tasks ADD CONSTRAINT tasks_task_type_check
  CHECK (task_type IN ('budget', 'tracking', 'cooking', 'subscription', 'reading', 'savings', 'no_spend', 'custom'));

-- Re-create preset challenges now that constraints are correct
DO $$
DECLARE
...
BEGIN
  -- Skip if challenges already exist
  IF EXISTS (SELECT 1 FROM challenges WHERE name = '30-Day Emergency Fund Sprint') THEN
    RAISE NOTICE 'Preset challenges already exist, skipping';
    RETURN;
  END IF;

  -- Use first real profile, or insert a system placeholder into auth.users + profiles
  SELECT id INTO organizer_id FROM profiles ORDER BY created_at ASC LIMIT 1;

  IF organizer_id IS NULL THEN
    -- Insert a system user into auth.users so the FK is satisfied
    INSERT INTO auth.users (id, email, created_at, updated_at, raw_app_meta_data, raw_user_meta_data, is_super_admin, encrypted_password)
      VALUES (system_user_id, 'system@tournacent.internal', now(), now(), '{}', '{}', false, '')
      ON CONFLICT (id) DO NOTHING;

    INSERT INTO profiles (id, display_name, created_at)
      VALUES (system_user_id, 'System', now())
      ON CONFLICT (id) DO NOTHING;
  ...
  INSERT INTO challenges (name, organizer_id, buy_in_amount, duration_days, start_date, end_date, status, prize_pool, created_at)
  VALUES ('30-Day Emergency Fund Sprint', organizer_id, 17.50, 30, now(), now() + interval '30 days', 'active', 0, now())
  RETURNING id INTO emergency_fund_id;

  -- Emergency Fund Sprint - Mandatory Tasks
  ...
  -- Emergency Fund Sprint - Optional Tasks
  INSERT INTO tasks (challenge_id, title, description, points, is_mandatory, task_type) VALUES
  ...
  INSERT INTO challenges (name, organizer_id, buy_in_amount, duration_days, start_date, end_date, status, prize_pool, created_at)
  VALUES ('No-Spend Reset Challenge', organizer_id, 15, 21, now(), now() + interval '21 days', 'active', 0, now())
  RETURNING id INTO no_spend_id;

  -- No-Spend Reset - Mandatory Tasks
  INSERT INTO tasks (challenge_id, title, description, points, is_mandatory, task_type) VALUES
  ...
  RAISE NOTICE 'Created preset challenges: Emergency Fund Sprint (%) and No-Spend Reset (%)', emergency_fund_id, no_spend_id;
END $$;
